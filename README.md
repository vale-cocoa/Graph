...
The three methods with signature `visitedVertices(adopting:reachableFrom:_:)` will proceed from the given `source` vertex on adjacencies only, by then returning a set containig all the vertices visited during the traversal. 
Thus these methods will stop after every vertex discoverable in the graph starting from the given source has been visited. 
